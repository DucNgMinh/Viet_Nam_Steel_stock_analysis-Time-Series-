...
# Step 3: fit model
```{r}
arima212 <- Arima(ts_closing, order = c(2, 1, 2), include.drift = TRUE, include.constant = TRUE)
summary(arima212)
```
...
```{r}
# ARIMA(1, 1, 1)
arima111 <- Arima(ts_closing, order = c(1, 1, 1), include.drift = TRUE, include.constant = TRUE)
summary(arima111)
autoplot(arima111)
...
```{r}
# ARIMA(2, 1, 1)
arima211 <- Arima(ts_closing, order = c(2, 1, 1), include.drift = TRUE, include.constant = TRUE)
summary(arima211)
autoplot(arima211)
