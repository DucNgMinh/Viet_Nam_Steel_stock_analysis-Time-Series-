...
```
```{r}
gross_sale_revenue <- ts(df, start = c(2008,1), frequency = 4)
...
```{r}
model2 <- lm(gross_sale_revenue ~ log(time))
summary(model2)
```
```{r}
graph_predict( model2, gross_sale_revenue, 2008, data_predict, 2, 5, 2023)
```
```{r}
# RMSE, MAPE all observation:
rmse_mape(model2, gross_sale_revenue)
```
```{r}
AIC(model2)
```
...
rmse_mape_4_last(model2, gross_sale_revenue)
```
...
```{r}
# RMSE, MAPE all observation:
rmse_mape(model3, gross_sale_revenue)
```
```{r}
AIC(model3)
```
```{r}
# RMSE, MAPE last 4 observations:
rmse_mape_4_last(model3, gross_sale_revenue)
```
...
model4 <- lm(gross_sale_revenue ~ s2 +s3 + s4)
summary(model4)
```
...
model5 <- lm(gross_sale_revenue ~ time + s2 +s3 + s4)
summary(model5)
```
```{r}
graph_predict(model5, gross_sale_revenue, 2008, data_predict, 2, 5, 2023)
```
```{r}
# RMSE, MAPE all observation:
rmse_mape(model5, gross_sale_revenue)
```
```{r}
AIC(model5)
```
```{r}
# RMSE, MAPE last 4 observations:
rmse_mape_4_last(model5, gross_sale_revenue)
```
...
```
```{r}
graph_predict(model9.seasadj, gross_sale_revenue, 2008, data_predict, 2, 5, 2023)
```
```{r}
rmse_mape(model9.seasadj, gross_sale_revenue)
rmse_mape_4_last(model9.seasadj, gross_sale_revenue)
```
```{r}
decom.gross_sale_revenue.m <- decompose(gross_sale_revenue, type = 'multiplicative')
decom.gross_sale_revenue.m.trend <- decom.gross_sale_revenue.m$trend
decom.gross_sale_revenue.m.seas <- decom.gross_sale_revenue.m$seasonal
decom.gross_sale_revenue.m.rand <- decom.gross_sale_revenue.m$rand
decom.gross_sale_revenue.m.seasadj <- gross_sale_revenue - decom.gross_sale_revenue.m$seasonal
plot(decom.gross_sale_revenue.m)
```
```{r}
plot(gross_sale_revenue, col = 'blue')
lines(decom.gross_sale_revenue.m.trend)
lines(decom.gross_sale_revenue.m.trendseas, col = 'red')
```
```{r}
model10.seasadj = lm(decom.gross_sale_revenue.m.seasadj ~ time )
```
```{r}
graph_predict(model10.seasadj, gross_sale_revenue, 2008, data_predict, 2, 5, 2023)
```
```{r}
rmse_mape(model10.seasadj, gross_sale_revenue)
rmse_mape_4_last(model10.seasadj, gross_sale_revenue)
```
