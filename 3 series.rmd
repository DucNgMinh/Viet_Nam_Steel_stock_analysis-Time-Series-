...
```
```{r}
testdata <- read_excel("C:\\Users\\ADMIN\\RProjects\\Viet_Nam_Steel_stock_analysis-Time-Series-\\Stock Price - HSG - test.xlsx")
p <- predict(var_diff_lag1, n.ahead= 10)
```
```{r}
HSG_test <- testdata$`<Close>`
```
```{r}
rmse(m.varf_lev_ft[500:509,2], HSG_test)
mape(m.varf_lev_ft[500:509,2], HSG_test) * 100
```
